...
export function AdminDashboard({ userId }: AdminDashboardProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSubmitMessage(error.message || "Failed to create content");
    ...
    <div className="bg-white rounded-xl shadow-sm border border-sand-200 p-6 md:p-8">
      <div className="mb-6">
        ...
        <p className="text-charcoal-600">
          Create new content for your Digital Sanctuary
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="w-5 h-5 text-bronze-500 border-sand-300 rounded focus:ring-bronze-400"
          />
          <label htmlFor="is_public" className="text-sm text-charcoal-700">
            Make this content public (visible to non-premium users)
          </label>
        </div>
